...
Follow this guide to migrate an old "coupled" menu to the new decoupled architecture.

## Step 1: Initialize the class
Ensure the class can be initialized with `options` and `ui`.
...
- Replace `Utilities.rofi_select` with `@ui.rofi_select`.

## Step 4: Handle Selection
The `rofi_select` method returns `nil` if the user cancels. Ensure this is handled.

```ruby
def show
  selected = @ui.rofi_select(items: @options.map { |i| i[:name] })
